...
(function () {
  ...
  function stats() {
    const s = rawStats();
    return { ok: s.ok || 0, n: s.n || 0, xp: s.xp || 0, miss: Array.isArray(s.miss) ? s.miss : [], seenRight: s.seenRight || {}, byCat: s.byCat || {}, byDiff: s.byDiff || {}, review: Array.isArray(s.review) ? s.review : [], streak: s.streak || 0 };
  }
  ...
  function toast(t) {
    var el = $("toast"); el.textContent = t; el.classList.add("show", "pop");
    clearTimeout(toast._t); toast._t = setTimeout(function () { el.classList.remove("show", "pop"); }, 1400);
  }
  ...
  function renderFromPrepared(prep, restore) {
    ...
    var d = q.difficulty || 2; var dName = d === 3 ? "HARD" : d === 1 ? "EASY" : "MED"; var dCls = d === 3 ? "hard" : d === 1 ? "easy" : "med";
    $("card").innerHTML = '<div class="chart"><div class="roomrow"><div class="qnum">' + (i + 1) + ' / ' + queue.length + '</div><div class="meta"><span class="tag">' + extra + '</span><span class="tag ' + dCls + '">' + dName + '</span></div></div>' + (q.case ? '<div class="case" id="caseBox">' + escapeHtml(q.case) + '</div>' : '') + '<p class="stem" id="stemBox">' + escapeHtml(q.stem) + '</p><div class="opts" id="optBox">' + prep.opts.map(function (o, idx) { return '<button class="opt" type="button" data-i="' + idx + '"><span class="ltr">' + letters()[idx] + '</span><span>' + escapeHtml(o.t) + '</span></button>'; }).join('') + '</div><div class="why" id="why"></div></div>';
    ...
    else { selected.forEach(function (n) { var el = $("optBox").children[n]; if (el) el.classList.add("selected"); }); $("mainBtn").textContent = "CHECK"; }
    paintHud(); $("hint").classList.toggle("hidden", !!ui().hintSeen);
  }
  function render() {
    if (!queue.length) {
  ...
  function paintGrade(ok) {
    var need = new Set(); rendered.opts.forEach(function (o, n) { if (o.k) need.add(n); });
    Array.prototype.forEach.call($("optBox").children, function (el, n) {
      if (need.has(n)) el.classList.add("correct"); else if (selected.has(n)) el.classList.add("wrong");
    });
    var why = $("why"); var nx = "";
    if (!ok) {
      var lines = [];
    ...
    why.classList.add('show'); $("mainBtn").textContent = 'NEXT';
  }
  function applyMissLogic(id, ok) {
    var s = stats(); var miss = new Set(s.miss); var seenRight = Object.assign({}, s.seenRight); var gained = 0; var beforeLv = levelOf(s.xp || 0);
    if (ok) { seenRight[id] = (seenRight[id] || 0) + 1; if (seenRight[id] >= 2) { miss.delete(id); delete seenRight[id]; } gained = XP_PER[rendered.raw.difficulty || 2] || 20; if (streak >= 3) gained += 5; }
    ...
    saveStats({ miss: Array.from(miss), seenRight: seenRight, byCat: byCat, byDiff: byDiff, review: review, ok: s.ok + (ok ? 1 : 0), n: s.n + 1, xp: xp });
    return { gained: gained, leveled: levelOf(xp) > beforeLv, level: levelOf(xp) };
  }
  function grade() {
    if (animating) return; if (locked) { next(1); return; } if (selected.size === 0) { toast('Pick an answer first'); return; }
    ...
    if (ok) { streak += 1; shiftCorrect += 1; shiftMaxStreak = Math.max(shiftMaxStreak, streak); } else streak = 0;
    var result = applyMissLogic(rendered.raw.id, ok);
    if (ok) { if (result.leveled) toast('LEVEL ' + result.level + '!'); else if (streak > 1) toast(streak + ' streak  +' + result.gained); else toast('+' + result.gained + ' XP'); live('Correct. ' + rendered.raw.why); }
    else { toast('Missed \u2014 saved'); live('Incorrect. ' + rendered.raw.why); }
    shiftLog.push({ id: rendered.raw.id, ok: ok, cat: rendered.raw.cat, stem: rendered.raw.stem }); paintHud();
  }
  ...
  function bankSkipMiss() {
    ...
    shiftLog.push({ id: rendered.raw.id, ok: false, cat: rendered.raw.cat, stem: rendered.raw.stem, skipped: true }); toast('Skipped \u2014 saved as missed'); live('Skipped and saved as missed.'); paintHud();
  }
  ...
  function back() {
    if (animating) return; var prev = history.pop(); if (!prev) { toast('First question'); return; }
  ...
  function hideOverlays() { $("gate").classList.add('hidden'); $("menu").classList.add('hidden'); $("debrief").classList.add('hidden'); $("field").classList.add('hidden'); }
  ...
  function showCaseDebrief() {
    ...
    $("debrief").className = 'debrief';
    $("debrief").innerHTML = '<h2>Story done.</h2><p class="muted">' + ok + '/' + n + ' on ' + escapeHtml(pack.title) + '</p><p class="muted">' + escapeHtml(pack.wrap || '') + '</p>' + (misses.length ? '<div class="miss-list">' + misses.map(function (m) { return '\u2022 ' + escapeHtml(m.stem); }).join('<br>') + '</div>' : '<p class="muted">Clean run.</p>') + '<div class="choice"><button type="button" id="dMix"><b>Keep playing</b></button><button type="button" id="dMiss"><b>Play missed</b></button></div>';
    $("dMix").onclick = function () { startMode('mix'); }; $("dMiss").onclick = function () { startMode('miss'); };
  }
  function showShiftDebrief() {
    ...
    $("debrief").className = 'debrief';
    $("debrief").innerHTML = '<h2>Quick 20 done.</h2><p class="muted">' + ok + '/' + n + ' \u00b7 ' + pct + '% \u00b7 best streak ' + shiftMaxStreak + '</p>' + (misses.length ? '<div class="miss-list">' + misses.slice(0, 8).map(function (m) { return '\u2022 ' + escapeHtml(m.stem); }).join('<br>') + '</div>' : '<p class="muted">No misses.</p>') + '<div class="choice"><button type="button" id="dMiss"><b>Play missed</b></button><button type="button" id="dMix"><b>Keep playing</b></button></div>';
    $("dMiss").onclick = function () { startMode('miss'); }; $("dMix").onclick = function () { startMode('mix'); };
  }
  ...
  function showMenu(openCases) {
    ...
    if (openCases) {
      ...
      box.innerHTML = Object.keys(pack).map(function (id) { return '<button type="button" data-case="' + id + '"><b>' + pack[id].title + '</b><span>6 questions</span></button>'; }).join('') || '<p class="muted">Stories still loading.</p>';
  ...
  function showField() {
    hideOverlays();
    var s = stats(); var acc = s.n ? Math.round(100 * s.ok / s.n) : 0; var lv = levelOf(s.xp);
    function lane(d, label) {
      var b = s.byDiff[String(d)] || { ok: 0, n: 0 }; var pct = b.n ? Math.round(100 * b.ok / b.n) : 0;
    ...
    var missHtml = missed.length ? missed.slice(0, 16).map(function (q) { return '<button type="button" class="bad" data-jump="' + q.id + '"><b>' + escapeHtml(clip(q.stem, 90)) + '</b><em>Tap to try again</em></button>'; }).join('') : '<p class="muted">No missed questions yet.</p>';
    $("field").className = 'field';
    $("field").innerHTML = '<p class="kicker">' + (track || '').toUpperCase() + ' \u00b7 Level ' + lv + '</p><h2>' + acc + '%</h2><div class="statrow"><div class="stat"><b>' + s.ok + '</b><span>correct</span></div><div class="stat"><b>' + s.miss.length + '</b><span>missed</span></div><div class="stat"><b>' + (s.xp || 0) + '</b><span>XP</span></div></div><p class="kicker">Play by level</p><div class="lanes">' + lane(0, 'All') + lane(1, 'Easy') + lane(2, 'Med') + '</div><div class="lanes" style="grid-template-columns:1fr 1fr">' + lane(3, 'Hard') + '<button class="lane" id="walkLane"><b>Play</b><span>Start this set</span></button></div><p class="kicker">Missed</p><div class="roomlist" id="missBox">' + missHtml + '</div><div class="choice"><button type="button" id="fMiss"><b>Play all missed</b></button><button type="button" id="fMix"><b>Back to play</b></button></div>';
    ...
    $("field").querySelectorAll('[data-jump]').forEach(function (b) {
      b.onclick = function () { var q = byId(b.dataset.jump); if (!q) return; mode = 'review'; queue = [q]; i = 0; hideOverlays(); render(); };
    });
  ...
  function isScrollableTarget(el) {
    var n = el; while (n && n !== document.body) { if (n.id === 'optBox' || n.id === 'stemBox' || n.id === 'caseBox' || n.id === 'why' || n.id === 'field') return true; n = n.parentElement; } return false;
  }
  function bindSwipe() {
    var startY = null, startX = null;
    function down(e) { var t = e.touches ? e.touches[0] : e; startY = t.clientY; startX = t.clientX; }
    function up(e) {
      if (startY == null) return; var t = e.changedTouches ? e.changedTouches[0] : e; var dy = startY - t.clientY; var dx = Math.abs(t.clientX - startX); startY = null;
      if (dx > 70) return; if (dy > 56) next(1); else if (dy < -56) back();
    }
  ...
  $("meBtn").onclick = showField;
  $("missChip").onclick = function () { startMode('miss'); };
